...
public class Exercise_05_02_Test {
	...
	@Points(points = 0.5, exerciseId = 2, subExId = 4, errorMessage = "vocabulary is not constructed correctly (small example)")

	@Test
	public void testVocab() throws Exception {
		Set<String> v = extractVocabulary(
				asList(text("A", "a", "b"), text("B", "a", "c"), text("C", "d", "e", "f"), text("A", "g", "h")));
		Set<String> expected = IntStream.range(0, 8).mapToObj(i -> "" + (char) ('a' + i)).collect(toSet());

		assertEquals(expected, v);
	}

	@Points(points = 0.5, exerciseId = 2, subExId = 4, errorMessage = "vocabulary is not constructed correctly (large example)")
	...
	@Test
	@Points(points = 1, exerciseId = 2, subExId = 4, errorMessage = "estimating class probabilities does not work")
	...
	@Test
	@Points(points = 1, exerciseId = 2, subExId = 4, errorMessage = "estimClassProb does not produce a probability distribution")
	public void testEstimateClassProbRandomized() {
		...
		for (int rep = 0; rep < 20; rep++) {
			...
			for (int i = 0; i < needInsts; i++) {
				insts.add(text(rnd.nextBoolean() ? "A" : rnd.nextBoolean() ? "B" : "C"));
			}
			Map<String, Double> est = estimateClassProbabilities(insts);
			checkProbDistr(est);
		}
	...
	private static void checkProbDistr(Map<String, Double> distr) {
		...
		for (Map.Entry<String, Double> kv : distr.entrySet()) {
			...
			assertTrue("probs must be <= 1 but is " + p, p <= 1);
			totalSum += p;
		}
	...
	@Test
	@Points(points = 1, exerciseId = 2, subExId = 4, errorMessage = "estimCondProb (small example) doesn't work")
	public void testEstimateCondProb() {
		List<TextInstance> insts = asList(text("A", "a", "b", "c"), text("B", "a", "b"), text("A", "a", "c", "d"));
		...
		for (int virtual = 0; virtual < 10; virtual++) {
			Map<String, Map<String, Double>> est = estimateConditionalWordProbs(insts, vocab, virtual);
			assertEquals(est.get("A").get("a"), (aA + virtual * prior) / (totalA + virtual), 0.0001);
			assertEquals(est.get("A").get("b"), (bA + virtual * prior) / (totalA + virtual), 0.0001);
			assertEquals(est.get("A").get("c"), (cA + virtual * prior) / (totalA + virtual), 0.0001);
			assertEquals(est.get("A").get("d"), (dA + virtual * prior) / (totalA + virtual), 0.0001);
			assertEquals(est.get("B").get("a"), (aB + virtual * prior) / (totalB + virtual), 0.0001);
			assertEquals(est.get("B").get("b"), (bB + virtual * prior) / (totalB + virtual), 0.0001);
			assertEquals(est.get("B").get("c"), virtual * prior / (totalB + virtual), 0.0001);
			assertEquals(est.get("B").get("d"), virtual * prior / (totalB + virtual), 0.0001);
		}
	}

	@Test
	@Points(points = 1, exerciseId = 2, subExId = 4, errorMessage = "estimCondProb doesn't produce probability distributions")
	public void testEstimateCondProbRandomized() {
		...
		for (int rep = 0; rep < 10; rep++) {
			...
			while (used.size() < vocabSize || insts.size() < needInsts) {
				...
				insts.add(text(rnd.nextBoolean() ? "A" : rnd.nextBoolean() ? "B" : "C", words));
			}

			for (int virtual = 0; virtual < 10; virtual++) {
				// compute conditional probability estimations
				Map<String, Map<String, Double>> est = estimateConditionalWordProbs(insts, vocabGen, virtual);
	...
	@Test
	@Points(points = 2, exerciseId = 2, subExId = 4, errorMessage = "final test doesn't work as expected")
	public void integrationTest() throws Exception {
		NaiveBayesTextClassifier c = learnNaiveBayesText(loadTrainingSet(), 1000);
		List<TextInstance> examples = Arrays.asList(textRaw("sci.space", "  The first step in embarking on a long and"
				...
				+ " the solar system.  will improve" + " and learn new ways to ensure astronauts are safe, healthy and"
				...
				+ " Space Station and low Earth orbit." + " " + " Why Translunar Space?" + " "
				...
				textRaw("comp.graphics",
						" Primitive assembly is the process of"
								...
								+ " arbitrary data that was output from the previous vertex or geometry" + " shader."
				...
				textRaw("rec.sport.baseball",
						" comes out of the bullpen to"
								...
								+ " baseman smacked a 2-1 fastball the other way to get the inning " + " going. " + "  "
				...
				textRaw("talk.politics.guns",
						" Holding aloft one of the banned weapons--an Israeli-made Galil"
								...
								+ " its ability to accept a bayonet lug, a pistol grip and a large" + " magazine."
								...
								+ " ban from any importers for 30 days. The final regulations would be" + " issued"),
				...
				textRaw("sci.med", " said this led to the development of monoclonal antibodies that"
						+ " were used in the UK to treat infected health workers returning" + " from Africa."
						+ " 'The down side is that monoclonal antibodies require considerable"
						+ " investment for scale-up and manufacture, and are expensive,'" + " Professor Khromykh said."
		...
		for (TextInstance example : examples) {
			...
			System.out.println("PREDICTED: " + predicted + " ACTUAL: " + example.getLabel());
